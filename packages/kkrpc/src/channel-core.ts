 ...
 *
 * Custom properties (`method`, `timeoutMs`) survive kkrpc's error
 * serialization round-trip (same mechanism
...
> {
	...
	private async handleIncomingMessage(raw: string | IoMessage): Promise<void> {
		...
		const message = await this.serializationRuntime.decodeMessage(payload)
	...
	private async handleMessageStr(messageStr: string): Promise<void> {
		this.count++
		try {
			const parsedMessage = await this.serializationRuntime.decodeMessage(messageStr)
			...
			if (error) {
				...
				if (typeof error === "object" && error.name && error.message) {
					this.pendingRequests[id].reject(
						this.serializationRuntime.deserializeError(error as EnhancedError)
					)
				} else {
	...
	private sendStreamError(requestId: string, error: Error): void {
		this.sendMessage({
			id: requestId,
			method: "",
			args: { error: this.serializationRuntime.serializeError(error) },
			type: "stream-error"
		})
	}
	...
	private handleStreamError(message: Message): void {
		...
		const error = isEnhancedError(rawError)
			? this.serializationRuntime.deserializeError(rawError)
	...
	private sendError(id: string, error: string | Error): void {
		const errorResponse = error instanceof Error ? this.serializationRuntime.serializeError(error) : error
	...
	private sendMessage(
		...
		const encoded = this.serializationRuntime.encodeMessage(
			message,
